...
class FileWithConstantWidth():
    ...
    def get_value(self, block: str, field: str, transaction_no=0):
        ...
        try:
            with open(self.path, "r") as file:
                for line in file:
                    if block_id == line[0:2] and self.transaction_check(block, line):
    ...
    def transaction_check(self, block_name, line, transaction_no=0):
        if block_name == "transaction":
            counter = str(self.transaction.get_field_value_from_line("Counter", line))
    ...
    def create_empty_file(self):
        ...
        new_file_content = self.header.create_empty_line() + "\n" + \
                    self.footer.create_empty_line() + "\n"
        self.drop_payload_to_file(new_file_content)
        # with open(self.path, "w") as file:
        #     file.write(self.header.create_empty_line() + "\n")
        #     file.write(self.footer.create_empty_line() + "\n")
...
class Field():
    ...
    def get_value_from_line(self, line: str) -> str:
        if len(line) != 121: # 120 + end of line
            raise ImportError
        return line[self.position_from-1: self.position_to]
